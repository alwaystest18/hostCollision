...
//发起http请求，增加请求头Host
func GetPageContentWithHost(urlStr string, hostName string) (string, int, error) {
	//display 'Unsolicited response received on idle HTTP channel starting with "\n"; err=<nil>' error
	log.SetOutput(ioutil.Discard) 
...
func HostCollision(urlStr string, hostsList []string) {
	...
	pageContent1, pageStatusCode1, err := GetPageContent(urlStr)
	if err != nil {
		return
	}
	for _, hostName := range hostsList {
		...
		pageContent2, pageStatusCode2, err := GetPageContentWithHost(urlStr, testHostName)
		if err != nil {
			continue
		}

		//对需要碰撞的host发起http请求，提取状态码与页面内容作为对比特征
		pageContent3, pageStatusCode3, err := GetPageContentWithHost(urlStr, hostName)
		if err != nil {
			continue
		}
